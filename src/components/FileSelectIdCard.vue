...
<template>
	<div class="border-4 box-border w-full rounded-md">
		...
		<div class="border-dashed box-border border-4 border-gray-500 bg-gray-100 bg-opacity-50  w-full h-40 flex flex-col-reverse rounded-xl text-center"
			v-loading="loading">
			<img v-if="hasSelected" :src="theFile.fc" class=" w-full h-40 object-scale-down z-0 rounded-xl"
				@click="onClick" />
			<img v-if="!hasSelected" :src="default_pic" class=" w-full h-40 object-scale-down z-0 rounded-xl"
				@click="onClick" />
		</div>
