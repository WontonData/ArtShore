...
<template>
  ...
  backdrop-filter backdrop-blur-2xl backdrop-saturate-10
		...
		<div class="flex-1 flex align-middle p-2 items-stretch  lg:max-w-[20rem] md:max-w-[5rem] sm:max-w-[5rem] cursor-pointer"
			@click="go_index">
		...
		<div v-else class="flex-1 flex justify-end">
			...
			<el-image class="h-8 menu-item  min-w-[3rem] sm:min-w-[2rem] lg:min-w-[4rem] max-w-[4.2rem]" :src="my_url"
				:fit="fit" ref="login" @click="go_profile"></el-image>
			<el-image class="menu-item h-8 min-w-[3rem] sm:min-w-[2rem] lg:min-w-[4rem]  max-w-[4.2rem]"
				:src="wallet_url" :fit="fit"></el-image>
		</div>
		<!-- <el-popover
        ref="poplogin"
        :virtual-ref="login"
