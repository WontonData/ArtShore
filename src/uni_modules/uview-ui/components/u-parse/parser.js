  ...
  while (i != -1) {
    ...
    if (str[i + 1] == '#') {
      ...
      if (config.entities[code] || (code == 'amp' && amp))
        str =
          str.substr(0, i) + (config.entities[code] || '&') + str.substr(j + 1)
    }
...
parser.prototype.parseStyle = function (node) {
  const { attrs } = node
  const list = (this.tagStyle[node.name] || '')
    .split(';')
    .concat((attrs.style || '').split(';'))
  const styleObj = {}
  let tmp = ''
  ...
  if (attrs.height) {
    styleObj.height =
      parseFloat(attrs.height) + (attrs.height.includes('%') ? '%' : 'px')
    attrs.height = void 0
  }
...
parser.prototype.onAttrName = function (name) {
  ...
  if (name.substr(0, 5) == 'data-') {
    ...
    if (name == 'data-src' && !this.attrs.src)
      this.attrName =
        'src' // a 和 img 标签保留 data- 的属性，可以在 imgtap 和 linktap 事件中使用
    else if (this.tagName == 'img' || this.tagName == 'a')
      this.attrName = name // 剩余的移除以减小大小
...
parser.prototype.onAttrVal = function (val) {
  ...
  if (name == 'style' || name == 'href')
    this.attrs[name] = decodeEntity(val, true) // 拼接主域名
  ...
  if (close) {
    if (!this.hook(node) || config.ignoreTags[node.name]) {
      ...
      if (node.name == 'base' && !this.options.domain)
        this.options.domain = attrs.href // #ifndef APP-PLUS-NVUE
          ...
          for (let i = this.stack.length; i--; ) {
            ...
            if (
              ...
              for (let _j = i + 1; _j < this.stack.length; _j++) {
                ...
                if (
                  !_style.includes(';width') &&
                  !_style.includes(' width') &&
                  _style.indexOf('width') != 0
                ) {
                  styleObj.width = ''
                  break
                }
          ...
          if (this.imgList.includes(_src)) {
            ...
            if (_i != -1) {
              ...
              for (; _i < _src.length; _i++) {
                if (_src[_i] == '/') break
                newSrc +=
                  Math.random() > 0.5 ? _src[_i].toUpperCase() : _src[_i]
              }
    ...
    for (const key in styleObj) {
      if (styleObj[key])
        attrs.style += ';'
          .concat(key, ':')
          .concat(styleObj[key].replace(' !important', ''))
    }
    ...
    if (
      node.name == 'pre' ||
      ((attrs.style || '').includes('white-space') &&
        attrs.style.includes('pre'))
    )
...
parser.prototype.onCloseTag = function (name) {
  ...
  if (i != -1) {
    ...
    const siblings = this.stack.length
...
parser.prototype.popNode = function () {
  ...
  if (config.blockTags[node.name])
    node.name = 'div' // 未知标签转为 span，避免无法显示
  else if (!config.trustTags[node.name] && !this.xml) node.name = 'span'
  if (
    node.name == 'a' ||
    node.name == 'ad' || // #ifdef H5 || APP-PLUS
    ...
    if (this.options.pauseVideo)
      str +=
        ' onplay="for(var e=document.getElementsByTagName(\'video\'),t=0;t<e.length;t++)e[t]!=this&&e[t].pause()"'
    str += '>'
      ...
      for (let row = 1; row <= trList.length; row++) {
        ...
        for (let j = 0; j < trList[row - 1].children.length; j++, col++) {
          ...
          if (td.name == 'td' || td.name == 'th') {
            ...
            const start = _style2.indexOf('width')
              ? _style2.indexOf(';width')
              : 0 // 提取出 td 的宽度
            ...
            _style2 +=
              (border
                ? ';border:'.concat(border, 'px solid gray') +
            ...
            if (td.attrs.colspan) {
              ...
              if (!td.attrs.rowspan)
                _style2 += ';grid-row-start:'
                  .concat(row, ';grid-row-end:')
                  .concat(row + 1)
            ...
            if (td.attrs.rowspan) {
              _style2 += ';grid-row-start:'
                .concat(row, ';grid-row-end:')
                .concat(row + parseInt(td.attrs.rowspan))
              if (!td.attrs.colspan)
                _style2 += ';grid-column-start:'
                  .concat(col, ';grid-column-end:')
                  .concat(col + 1) // 记录下方单元格被占用
        ...
        ;(function traversal(nodes) {
          for (let _i7 = 0; _i7 < nodes.length; _i7++) {
            ...
            if (_td.name == 'th' || _td.name == 'td') {
              if (border)
                _td.attrs.style = 'border:'
                  .concat(border, 'px solid gray;')
                  .concat(_td.attrs.style || '')
              if (padding)
                _td.attrs.style = 'padding:'
                  .concat(padding, 'px;')
                  .concat(_td.attrs.style || '')
  ...
  const flex =
    parent &&
    (parent.attrs.style || '').includes('flex') && // #ifdef MP-WEIXIN
  ...
  for (const key in styleObj) {
    if (styleObj[key]) {
      const val = ';'
        .concat(key, ':')
        .concat(styleObj[key].replace(' !important', '')) // #ifndef APP-PLUS-NVUE
...
lexer.prototype.checkClose = function (method) {
  ...
  if (
    this.content[this.i] == '>' ||
    (selfClose && this.content[this.i + 1] == '>')
  ) {
