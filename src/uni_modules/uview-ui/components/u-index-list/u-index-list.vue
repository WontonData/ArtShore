...
export default {
  ...
  computed: {
    ...
    // 字母放大指示器的top值，为了让其指向当前激活的字母
    indicatorTop() {
      const { top, itemHeight } = this.letterInfo
      return Math.floor(
        top +
          itemHeight * this.activeIndex +
          itemHeight / 2 -
  ...
  methods: {
    ...
    setValueForTouch(currentIndex) {
      ...
      this.scrollIntoView = `u-index-item-${this.uIndexList[
        currentIndex
      ].charCodeAt(0)}`
      // #endif
      // #ifdef MP-WEIXIN
    ...
    async scrollHandler(e) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (
          scrollTop <= children[0].top ||
          scrollTop >= children[len - 1].top + children[len - 1].height
        ) {
          this.activeIndex = -1
          break
        } else if (!nextItem) {
