  ...
  methods: {
    ...
    moveSwipeAction(moveX) {
      ...
      animation.transition(
        this.$refs['u-swipe-action-item__content'].ref,
        {
          styles: {
      ...
      for (let i = len - 1; i >= 0; i--) {
        const buttonRef =
          this.$refs[`u-swipe-action-item__right__button-${i}`][0].ref
        // 通过比例，得出元素自身该移动的距离
    ...
    openSwipeAction() {
      ...
      for (let i = len - 1; i >= 0; i--) {
        ...
        // 通过比例，得出元素自身该移动的距离
        const translateX =
          (this.buttons[i].width / this.buttonsWidth) * buttonsWidth
        // 最终移动的距离，是通过自身比例算出的距离，再加上在它之前所有按钮移动的距离之和
        const realTranslateX = translateX + previewButtonsMoveX
        animation.transition(
          buttonRef,
    ...
    queryRect() {
      ...
      const promiseAll = this.rightOptions.map((item, index) =>
        this.getRectByDom(
          this.$refs[`u-swipe-action-item__right__button-${index}`][0]
        )
      )
