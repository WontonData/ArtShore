  ...
  methods: {
    ...
    change(e) {
      ...
      if (this.mode === 'time') {
        ...
        selectValue = `${this.intercept(values[0][indexs[0]])}:${this.intercept(
          values[1][indexs[1]]
        )}`
      } else {
        ...
        let date = parseInt(
          values[2] ? this.intercept(values[2][indexs[2]]) : 1
        )
        let hour = 0,
          minute = 0
        // 此月份的最大天数
    ...
    updateIndexs(value) {
      ...
      if (this.mode === 'time') {
        ...
        values = [
          formatter('hour', timeArr[0]),
          formatter('minute', timeArr[1]),
        ]
      } else {
    ...
    correctValue(value) {
      ...
      if (!isDateMode) {
        ...
        hour = uni.$u.padZero(
          uni.$u.range(this.minHour, this.maxHour, Number(hour))
        )
        minute = uni.$u.padZero(
          uni.$u.range(this.minMinute, this.maxMinute, Number(minute))
        )
        return `${hour}:${minute}`
      } else {
        // 如果是日期格式，控制在最小日期和最大日期之间
        value = dayjs(value).isBefore(dayjs(this.minDate))
          ? this.minDate
          : value
